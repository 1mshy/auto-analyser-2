...
use auto_analyser_2::symbols::normalize_symbol_key;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut stocks: Vec<(String, String, f64)> = nasdaq_response
        ...
        .filter_map(|stock| {
            ...
            if !stock.symbol.is_empty() && market_cap > 0.0 {
                Some((normalize_symbol_key(&stock.symbol), stock.name, market_cap))
            } else {
                None
            }
        })
    ...
    println!(
        "✓ Filtered to {} stocks with valid market cap\n",
        stocks.len()
    );
    ...
    println!("  1. Fetch US stocks by market cap from NASDAQ");
    println!("  2. Normalize symbols to Yahoo/Mongo keys");
    println!("  3. Merge configured Canadian Yahoo tickers via CANADIAN_SYMBOLS");
    println!("  4. Cache the list in memory and fall back if the API fails");
