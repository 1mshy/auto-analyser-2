...
impl AnalysisEngine {
    ...
    pub async fn start_continuous_analysis(&self) {
        ...
        info!(
            ...
            self.yahoo_concurrency, self.yahoo_delay_ms
        );
    ...
    async fn run_analysis_cycle(&self) -> anyhow::Result<()> {
        ...
        {
            let mut progress = self.progress.write().await;
            progress.last_cycle_started = Some(cycle_started);
            progress.cycle_start = cycle_started;
        ...
        info!(
            "📊 Analyzing {} stocks ({} skipped, already up-to-date)",
            total_to_analyze, skipped
        );
        ...
        if symbols_to_analyze.is_empty() {
            ...
            progress.current_symbol = None;
            progress.last_cycle_completed = Some(completed);
            progress.last_successful_cycle = Some(completed);
            return Ok(());
        }
        ...
        if let Err(e) = fetch_handle.await {
            ...
            let mut progress = self.progress.write().await;
            progress.errors = error_count;
            progress.last_error = Some(format!("fetch task failed: {}", e));
            return Err(anyhow::anyhow!("fetch task failed: {}", e));
        }
    ...
    async fn process_stock_with_prices(
        ...
        let latest_price = historical_prices
            .last()
        ...
        if let Some(pct) = quote.price_change_percent {
            if pct.abs() > self.max_abs_price_change_percent {
                return Err(anyhow::anyhow!(
                    ...
                    symbol,
                    pct,
                    self.max_abs_price_change_percent
                ));
            }
        }

        // Fetch news only for symbols likely to matter in the feed. Detail/API
        // routes can still fetch on demand; the 24/7 cycle should not burst
        // thousands of NASDAQ headline calls.
        let should_fetch_news = is_notable_for_news(market_cap, rsi, quote.price_change_percent);
    ...
    async fn get_stock_symbols(&self) -> Vec<(String, Option<f64>)> {
        ...
        match self.fetch_nasdaq_stocks().await {
            ...
            Err(e) => {
                warn!(
                    "Failed to fetch NASDAQ stocks: {}. Using cached/fallback list.",
                    e
                );
                ...
                if !cached.is_empty() {
                    ...
                    vec![
                        ...
                        "JNJ", "V", "PG", "UNH", "HD", "MA", "DIS", "PYPL", "NFLX", "ADBE", "CRM",
                        "CSCO", "INTC", "PFE", "VZ", "KO", "NKE", "MRK", "T", "PEP", "ABT", "TMO",
                        "COST", "AVGO", "ACN", "DHR", "TXN", "NEE", "LLY", "MDT", "ORCL", "WMT",
                        "HON", "PM", "UNP", "BMY", "QCOM", "C", "LOW", "UPS", "RTX", "BA", "AMGN",
                        "IBM", "SBUX", "CAT", "GE", "AMD", "GILD", "CVS", "MMM", "MO", "USB",
                        "TGT",
                    ]
                    .iter()
    ...
    async fn fetch_nasdaq_stocks(&self) -> anyhow::Result<Vec<(String, Option<f64>)>> {
        ...
        let mut stocks: Vec<(String, Option<f64>)> = nasdaq_response
            ...
            .filter_map(|stock| {
                ...
                Some((
                    crate::symbols::normalize_symbol_key(&stock.symbol),
                    Some(mc),
                ))
            })
            .collect();
        ...
        let mut seen: std::collections::HashSet<String> =
            stocks.iter().map(|(symbol, _)| symbol.clone()).collect();
        for symbol in &self.canadian_symbols {
            if seen.insert(symbol.clone()) {
                stocks.push((symbol.clone(), None));
            }
        }
...
fn is_notable_for_news(
    market_cap: Option<f64>,
    rsi: Option<f64>,
    price_change_percent: Option<f64>,
) -> bool {
    market_cap.is_some_and(|cap| cap >= 10_000_000_000.0)
        || rsi.is_some_and(|value| !(30.0..=70.0).contains(&value))
        || price_change_percent.is_some_and(|pct| pct.abs() >= 5.0)
}
...
fn resolve_quote(
    ...
    if let Some(tech) = technicals {
        ...
        if let Some(prev_close) = tech
            .previous_close
            .filter(|prev_close| valid_price(*prev_close))
        {
...
mod tests {
    ...
    fn test_nasdaq_screener_filter_pipeline() {
        ...
        let rows = vec![
            NasdaqStock {
                ...
                market_cap: "3,400,000,000,000".to_string(),
            },
            NasdaqStock {
                symbol: "".to_string(),
                name: "Empty".to_string(),
                market_cap: "1,000,000".to_string(),
            },
            NasdaqStock {
    ...
    fn test_is_junk_symbol_rejects_warrants_and_units() {
        for junk in &[
            "FOO.W", "FOO.WS", "FOO/WS", "FOO-W", "FOO.WSA", "FOO.WSB", "FOO.U", "FOO-U", "FOO.UN",
            "FOO.R", "FOO-R", "FOO.RT", "foo.w", "foo.ws", // case-insensitive
        ] {
    ...
    fn test_is_junk_symbol_keeps_common_stock() {
        for good in &[
            "AAPL", "MSFT", "GOOG", "BRK-B", "BF-B", "HEI-A", "GOOG.L", "FOO.TO", "FOO-K", "FOO.C",
        ] {
    ...
    fn test_screener_filter_with_min_market_cap_and_junk() {
        ...
        let rows = vec![
            NasdaqStock {
                symbol: "AAPL".into(),
                name: "Apple".into(),
                market_cap: "3,400,000,000,000".into(),
            },
            NasdaqStock {
                symbol: "MSFT".into(),
                name: "Microsoft".into(),
                market_cap: "3,000,000,000,000".into(),
            },
            NasdaqStock {
                symbol: "SPAC.U".into(),
                name: "SPAC Unit".into(),
                market_cap: "500,000,000".into(),
            },
            NasdaqStock {
                symbol: "XYZ.WS".into(),
                name: "Warrant".into(),
                market_cap: "100,000,000".into(),
            },
            NasdaqStock {
                symbol: "MICRO".into(),
                name: "Micro-cap".into(),
                market_cap: "50,000,000".into(),
            },
            NasdaqStock {
                symbol: "ZERO".into(),
                name: "Zero".into(),
            ...
            NasdaqStock {
                symbol: "BRK-B".into(),
                name: "Berkshire".into(),
                market_cap: "1,000,000,000,000".into(),
            },
        ];
